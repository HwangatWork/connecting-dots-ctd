"""
시장 서비스 — 시장 탭 전체 데이터 조립.
providers에서 원시 데이터를 수집하고 스코어 엔진으로 온도 계산.
"""
...
from backend.providers import fear_greed as fg_p
from backend.services import technical as ta
from backend.services.score_engine import calculate_market_temperature, temperature_to_verdict
from backend.config import INDEX_SYMBOLS
...
# ── 가중치 상수 ────────────────────────────────────────────────
_HY_SPREAD_FALLBACK = 3.8   # 실시간 HY 스프레드는 별도 유료 API 필요 → 최근값 하드코딩
    ...
    nodes = [
        CTDNode(
            ...
            rows=[
                ...
                CTDRow(text=f"미국 10Y 금리 {macro.get('vix',4.45):.2f}% — 고점 대비 안정화", tag="긍정", cls="t-bull"),
