...
- **관리 페이지**: https://connecting-dots-ctd.netlify.app/status.html (패스코드: adminjy)
...
| 배포 | Render.com (백엔드) + Netlify (프론트) |
| MCP | Render MCP (`claude mcp list`에서 ✓ Connected 확인) |
...
├── CLAUDE.md               # 이 파일
├── ROADMAP.md              # 기능 로드맵
└── netlify.toml            # Netlify 배포 설정 (Render 설정은 대시보드 직접 관리)
```
...
2. **완료 기준**: GitHub push + **Netlify/Render 실제 URL 검증** 통과까지가 완료.
...
# 프론트엔드
# frontend/index.html 브라우저에서 직접 열거나
# Render URL로 API 호출 (index.html 내 API_BASE 자동 감지)
```
